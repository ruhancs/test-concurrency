...
func (app *Config) listenForMail() {
	for {
		select {
			//escutar o canal de emails
		case msg := <- app.Mailer.MailerChan:
			//enviar o email em background
			go app.Mailer.sendMail(msg,app.Mailer.ErrorChan)
		//escutar o canal com messages de erro ao enviar email
		case err := <- app.Mailer.ErrorChan:
			app.ErrorLog.Println(err)
		//escutar o canal de termino de envio de email
...
func (m *Mail) sendMail(msg Message, erroChan chan error) {
	defer m.Wait.Done()
	if msg.Template == "" {
		msg.Template = "mail"
	}
	...
	if err != nil {
		//envia o erro para o canal de erros
		erroChan <- err
	}
